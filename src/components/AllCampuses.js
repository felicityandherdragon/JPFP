...
class AllCampuses extends React.Component {
	render() {
		const { campuses: campusesOnPage } = this.props;
			...
			<div id='display'>
				<div className='view-controls'>
					...
					<ul className='filters'>
						<li
							onClick={() => this.props.fetchCampuses()}
							className={
						...
						<li
							onClick={() => this.props.setVisibility('SHOW_EMPTY_CAMPUSES')}
							className={
								this.props.visibilityFilter === 'SHOW_EMPTY_CAMPUSES'
									? 'active'
									: undefined
							}>
					...
					<ul className='sorting'>
						<li
							onClick={() => this.props.sortBy('BY_NUM_OF_STUDENTS')}
							className={
								this.props.sortFilter === 'BY_NUM_OF_STUDENTS'
									? 'active'
									: undefined
							}>
							Number of students
						...
						return (
							...
							>
								<div className='info'>
									<Link
										to={'/campuses/' + each.id}
										onClick={() => this.props.selectCampus(each.id)}>
										<h3>{each.name}</h3>
										<h4>🧭 Address</h4>
										<p>{each.address}</p>
										<h4>💬 Description</h4>
								...
								<button
									className='delete'
									onClick={() => this.props.deleteCampus(each)}>
									Delete
								</button>
							</li>
						);
